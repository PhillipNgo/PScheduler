...
public class ScheduleMaker {
    ...
    private void setTermYear(String termYear) throws Exception {
        ...
        String[] options = form.getSelectOptions("TERMYEAR");
        String[] optionValues = form.getSelectOptionValues("TERMYEAR");
        for (int i = 0; i < options.length; i++) {
            if (termYear.equals(options[i])) {
                term = optionValues[i];
                form.fillSelectField("TERMYEAR", term);
                return;
            }
        }
        if (termYear == null) {
     ...
     */
    private LinkedList<Class> parseClasses(String text) {
        LinkedList<Class> list = new LinkedList<Class>();
            ...
            while (count < 12 && i < values.length-1) {
                ...
                switch (count) {
                    ...
                    case 3: name = currString;
                            break;
                    case 4: type = currString;
                            if (type.equals("ONLINE") || type.equals("I") || type.equals("R")) {
                                count = 12;
                                i += 6;
                            }
                            if ((type.equals("L") || type.equals("B")) && !TimeSlot.isTime(values[i+5])) {
                                type = "I";
                                count = 12;
                                i += 6;
                            }
